...
function mapToTreatedLineData(detailedCrypto: CryptoDetail): LineChartData[] {
    ...
    return detailedCrypto.total_volumes.map(data => ({
        Volume: data[ 1 ],
        name: capitalizeFirstLetter(new Date(data[ 0 ]).toLocaleDateString(locale, { day: 'numeric', month: 'short' })),
    }));
}
...
onMounted(async () => {
    ...
    treatedLineData.value = mapToTreatedLineData(cryptoCurrencyStore.getCryptoDetail(crypto) as CryptoDetail);

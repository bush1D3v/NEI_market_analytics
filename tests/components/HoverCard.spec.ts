...
describe("HoverCard Component Tests", () => {
	const wrapper = shallowMount(HoverCard, {props: {...HoverCardMock}});
	...
	test("Should be able to render the component with the correct props", async () => {
		for (const attr in hoverCardComponent.vm.$attrs) {
			expect(hoverCardComponent.vm.$attrs[attr as keyof HoverCardProps]).toBe(
				HoverCardMock[attr as keyof HoverCardProps],
			);
		}
	});
